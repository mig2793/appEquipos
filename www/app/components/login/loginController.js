	...
	$scope.login = function(data){
		...
		datasend = JSON.stringify(data);
      	loguinService.servicesLogin(data,url).then(function(promise){
			...
			if(result.state == 1 && result.message[0].rol == "Usuario" && result.message[0].estado == 2){
				...
				if(result.message[0].rol != "Usuario" || result.message[0].estado == 1){
		            globals.set("El usuario no tiene permisos para ingresar a la aplicación o tu cuenta aún no ha sido activada.");
		            Modal.showModal({
